<script lang="ts">
	...
	let activeTab = $state('administration');
...
<div class="p-6">
	...
	<div class="flex border-b border-gray-200 mb-6">
		{#each tabs as tab}
			<button
				...
				class="px-4 py-2 font-medium transition-colors {activeTab === tab.id ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}"
			>
				{tab.label}
			</button>
		{/each}
	...
	<div class="bg-white shadow">
			...
			<div class="p-6">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{#each administrationItems as item}
						<a href={item.href} class="flex items-center gap-3 p-4 border border-gray-200 hover:bg-gray-50 transition-colors">
		...
		{:else if activeTab === 'security'}
			<div class="p-6">
				<h2 class="text-lg font-semibold text-gray-800 mb-4">Security & Access</h2>
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
